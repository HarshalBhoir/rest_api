...
from odoo.addons.rest_api.common import (
    extract_arguments,
    invalid_response,
    valid_response,
)
...
class APIController(http.Controller):
    ...
    @validate_token
    @http.route("/create_user", type="http", auth="none", methods=["POST"], csrf=False)
    def create_user(self,  **payload):
        ...
        print(payload)
        ioc_name = model_val
        model = request.env[self._model].search([("model", "=", model_val)], limit=1)
        values = {}
        if model:
            try:
                # changing IDs from string to int.
